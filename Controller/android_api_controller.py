...
def predict():
    ...
    # Validate inputs
    missing_fields = [key for key, value in input_data.items() if value is None or (isinstance(value, str) and not value.strip())]
    invalid_types = []
    for key in ['age', 'trestbps', 'chol', 'thalach', 'oldpeak']:
        try:
            input_data[key] = float(input_data[key]) if input_data[key] is not None else None
        except (ValueError, TypeError):
            invalid_types.append(key)
    for key in ['ca', 'sex', 'cp', 'exang', 'slope', 'thal']:
        try:
            input_data[key] = int(input_data[key]) if input_data[key] is not None else None
        except (ValueError, TypeError):
            invalid_types.append(key)
    ...
    try:
        ...
        if model_type == "Cluster":
            ...
            prediction = model.Predict(input_data)  # Pass dictionary per HeartClusterModel
            cluster = prediction['cluster']
            risk_level = prediction['risk_level']
        elif model_type == "Neural Network":
            model = NeuralNetworkModel()
            prediction = model.Predict(input_data)  # Pass dictionary per NeuralNetworkModel
            cluster = prediction['cluster']
            risk_level = prediction['risk_level']
        else:
